...
const renderResepi = async () => {
  ...
  // ---------
  // get food with id
  const food = await foodWithId(id);
  // ---------
  // set information
  ...
  const materials = await foodMaterials(food.id);
  // ----------
  let priceAll = 0;
  let Weight = 0;
  ...
  materials.forEach(async (item) => {
    ...
    removeItem.forEach((item) => {
      item.addEventListener("click", () => {
        DeleteMaterial(item.getAttribute("data-id"))
        item.parentNode.parentNode.remove();
      });
    });
  });
  ...
  name_material.forEach((item) => {
    ...
    console.log(InsertMaterialFood(data2));
...
removeBtn.forEach((item) => {
  item.addEventListener("click", async () => {
    console.log(await DeleteMaterial(item.getAttribute("data-id")));
