...
for (const element of monthLetter) {
	element.onclick = function (e) {
		...
		localStorage.setItem("mounth",monthDataNumber)
	}
}
...
for (const month of calendarObject) {
	...
	for (const day of month) {
		...
		if (!oneStarted){
			liClass += "disable-one ";
			let test = document.querySelectorAll(".disable-one")
			test.forEach(item=>{
...
function generateTemplateHTML(type, data) {
	...
	if (type == "date")
		htmlTemplate = `<li class="${data.class}"data-day="${data.jalali}" data-mounth="${data.mounth}" id="day"><span id="jalali">${data.jalali.convertDigits("fa")}</span><small id="miladi">${data.miladi}</small><small id="ghamari">${data.ghamari}</small></li>`;
...
let dates=0;
let jalal = document.querySelectorAll("#day")
jalal.forEach(item=>{
	item.addEventListener('click',()=>{
		const day = item.getAttribute("data-day")
		const mounth = item.getAttribute("data-mounth")
		dates = new Date(`1401-${mounth}-${day}`)
		console.log(dates);
		// date = new Intl.DateTimeFormat('en-US').format(dates);

	})
})
const submitBTM = document.querySelector("#submit")
submitBTM.addEventListener('click',async()=>{
	const title = document.querySelector("#title")
	const CounsellingType=document.querySelector("#noe-moshavereh")
	const job = document.querySelector("#jobs")
	const city = document.querySelector("#city")
	if(dates==0){
		successAlert('error','لطفا تاریخ مشاوره خود را مشخص کنید')
	}else{
		const data = {
			"topic":title.value,
			"job":job.value,
