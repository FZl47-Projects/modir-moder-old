import {PreparationsFoods,PreparationsfoodMaterials,PreparationsDeleteFoodsUser} from "./api.js"
...
const renderRawMaterial = async ()=>{
    const food =await PreparationsFoods(userid)
    food.forEach(async item => {
        const material = await PreparationsfoodMaterials(item.id)
        ...
        <a href="./UpdatePreparations.html?id=${item.id}">
          <div class="respi">
            مراحل آماده سازی
          </div>
        </a>
  ...
  remove.forEach(item=>{
    item.addEventListener('click', ()=>{
      console.log( PreparationsDeleteFoodsUser(item.getAttribute("data-id")))
